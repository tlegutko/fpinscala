...
object MyModule {
  ...
  def fib(n: Int): Int = {
    @annotation.tailrec
    def go(curr: Int, next: Int, cnt: Int): Int = {
      if (cnt == 0) curr else go(next, curr + next, cnt-1)
    }
    go(0, 1, n)
  }
...
object PolymorphicFunctions {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(isSorted(Array(1, 2, 5, 3), ascendingSort))
    println(isSorted(Array(1, 2, 3, 5), ascendingSort))
    println(isSorted(Array(1, 2, 2, 3), ascendingSort))
  }
